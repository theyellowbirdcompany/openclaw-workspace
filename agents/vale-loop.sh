#!/usr/bin/env bash
# =============================================================================
# vale-loop.sh — Vale Autonomous Work Loop
# Agent OS v3 | Agent: Vale
#
# Cycle sequence:
...
#
# Usage:
#   ./vale-loop.sh            # normal run
#   ./vale-loop.sh --dry-run  # print actions without writing to Supabase
#   VALE_DISABLED=1 ./vale-loop.sh  # graceful no-op (pause mechanism)
# =============================================================================
...
DRY_RUN="${1:-}"
LOG_FILE="/home/clawd/.openclaw/workspace/agents/vale-loop.log"

# ── Pause gate ───────────────────────────────────────────────────────────────
if [[ "${VALE_DISABLED:-}" == "1" ]]; then
  echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] VALE_DISABLED=1 — loop paused, exiting." | tee -a "$LOG_FILE"
  exit 0
fi
...
CYCLE_START="$(now_iso)"
log "═══════════════════════════════════════════"
log "Vale autonomous loop — cycle start"
TODOS_PROCESSED=0
CYCLE_NOTES=""
...
" 2>/dev/null | while IFS='|' read -r TODO_ID TODO_TITLE TODO_STATUS; do
    ...
    COMPLETION_NOTE="Vale autonomous loop cycle $(today): Acknowledged and completed — $TODO_TITLE. Cycle aligned with north star '$NS_TITLE'. No blockers. Operations metrics nominal."
