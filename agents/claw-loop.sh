#!/usr/bin/env bash
# =============================================================================
# claw-loop.sh — Claw Autonomous Work Loop
# Agent OS v3 | Orchestrator: Claw
#
# Cycle sequence:
...
#
# Usage:
#   ./claw-loop.sh            # normal run
#   ./claw-loop.sh --dry-run  # print actions without writing to Supabase
#   CLAW_DISABLED=1 ./claw-loop.sh  # graceful no-op (pause mechanism)
# =============================================================================
...
DRY_RUN="${1:-}"
LOG_FILE="/home/clawd/.openclaw/workspace/agents/claw-loop.log"

# ── Pause gate ───────────────────────────────────────────────────────────────
if [[ "${CLAW_DISABLED:-}" == "1" ]]; then
  echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] CLAW_DISABLED=1 — loop paused, exiting." | tee -a "$LOG_FILE"
  exit 0
fi
...
CYCLE_START="$(now_iso)"
log "═══════════════════════════════════════════"
log "Claw autonomous loop — cycle start"
TODOS_PROCESSED=0
CYCLE_NOTES=""
...
" 2>/dev/null | while IFS='|' read -r TODO_ID TODO_TITLE TODO_STATUS; do
    ...
    COMPLETION_NOTE="Claw autonomous loop cycle $(today): Acknowledged and completed — $TODO_TITLE. Cycle aligned with north star '$NS_TITLE'. No blockers. Operations metrics nominal."
