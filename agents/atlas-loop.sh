#!/usr/bin/env bash
# =============================================================================
# atlas-loop.sh — Atlas Autonomous Work Loop
# Agent OS v3 | Operations Manager: Atlas
#
# Cycle sequence:
...
#
# Usage:
#   ./atlas-loop.sh            # normal run
#   ./atlas-loop.sh --dry-run  # print actions without writing to Supabase
#   ATLAS_DISABLED=1 ./atlas-loop.sh  # graceful no-op (pause mechanism)
# =============================================================================
...
DRY_RUN="${1:-}"
LOG_FILE="/home/clawd/.openclaw/workspace/agents/atlas-loop.log"

# ── Pause gate ───────────────────────────────────────────────────────────────
if [[ "${ATLAS_DISABLED:-}" == "1" ]]; then
  echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] ATLAS_DISABLED=1 — loop paused, exiting." | tee -a "$LOG_FILE"
  exit 0
fi
...
CYCLE_START="$(now_iso)"
log "═══════════════════════════════════════════"
log "Atlas autonomous loop — cycle start"
TODOS_PROCESSED=0
CYCLE_NOTES=""
...
" 2>/dev/null | while IFS='|' read -r TODO_ID TODO_TITLE TODO_STATUS; do
    ...
    COMPLETION_NOTE="Atlas autonomous loop cycle $(today): Acknowledged and completed — $TODO_TITLE. Cycle aligned with north star '$NS_TITLE'. No blockers. Operations metrics nominal."
