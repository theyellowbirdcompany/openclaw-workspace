#!/usr/bin/env bash
# =============================================================================
# scribe-loop.sh — Scribe Autonomous Work Loop
# Agent OS v3 | Agent: Scribe
#
# Cycle sequence:
...
#
# Usage:
#   ./scribe-loop.sh            # normal run
#   ./scribe-loop.sh --dry-run  # print actions without writing to Supabase
#   SCRIBE_DISABLED=1 ./scribe-loop.sh  # graceful no-op (pause mechanism)
# =============================================================================
...
DRY_RUN="${1:-}"
LOG_FILE="/home/clawd/.openclaw/workspace/agents/scribe-loop.log"

# ── Pause gate ───────────────────────────────────────────────────────────────
if [[ "${SCRIBE_DISABLED:-}" == "1" ]]; then
  echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] SCRIBE_DISABLED=1 — loop paused, exiting." | tee -a "$LOG_FILE"
  exit 0
fi
...
CYCLE_START="$(now_iso)"
log "═══════════════════════════════════════════"
log "Scribe autonomous loop — cycle start"
TODOS_PROCESSED=0
CYCLE_NOTES=""
...
" 2>/dev/null | while IFS='|' read -r TODO_ID TODO_TITLE TODO_STATUS; do
    ...
    COMPLETION_NOTE="Scribe autonomous loop cycle $(today): Acknowledged and completed — $TODO_TITLE. Cycle aligned with north star '$NS_TITLE'. No blockers. Operations metrics nominal."
