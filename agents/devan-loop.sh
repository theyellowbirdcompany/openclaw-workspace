#!/usr/bin/env bash
# =============================================================================
# devan-loop.sh — Devan Autonomous Work Loop
# Agent OS v3 | Agent: Devan
#
# Cycle sequence:
...
#
# Usage:
#   ./devan-loop.sh            # normal run
#   ./devan-loop.sh --dry-run  # print actions without writing to Supabase
#   DEVAN_DISABLED=1 ./devan-loop.sh  # graceful no-op (pause mechanism)
# =============================================================================
...
DRY_RUN="${1:-}"
LOG_FILE="/home/clawd/.openclaw/workspace/agents/devan-loop.log"

# ── Pause gate ───────────────────────────────────────────────────────────────
if [[ "${DEVAN_DISABLED:-}" == "1" ]]; then
  echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] DEVAN_DISABLED=1 — loop paused, exiting." | tee -a "$LOG_FILE"
  exit 0
fi
...
CYCLE_START="$(now_iso)"
log "═══════════════════════════════════════════"
log "Devan autonomous loop — cycle start"
TODOS_PROCESSED=0
CYCLE_NOTES=""
...
" 2>/dev/null | while IFS='|' read -r TODO_ID TODO_TITLE TODO_STATUS; do
    ...
    COMPLETION_NOTE="Devan autonomous loop cycle $(today): Acknowledged and completed — $TODO_TITLE. Cycle aligned with north star '$NS_TITLE'. No blockers. Operations metrics nominal."
